...
$.oFolder.prototype.copy = function( folderPath, copyName, overwrite ){
    ...
    var nextFolder = new $.oFolder(folderPath.path+"/"+this.name);
    nextFolder.create();
    var files = this.getFiles();
    for(var index in files){
      var targetFile = new $.oFile(nextFolder.path+"/"+files[index].fullName);
      if(!targetFile.exists) files[index].copy(nextDir,undefined,false);
      else log("couldn't copy "+files[index].path+" to "+nextFolder.path);
    }
    var folders = this.getFolders();
    for(var index in folders){
      folders[index].copy(nextFolder,overwrite);
    }
}
